...
class WishListView(APIView):
    ...
    def get(self,request):
        try:
            wishlist = WhishList.objects.filter(user = request.user).exclude(is_deleted=True)
...
class BookingView(APIView):
    ...
    def get(self,request):
        ...
        if request.user.user_type == "customer":
            requests = BookingRequest.objects.filter(user=request.user,is_used = False,).exclude(status = "rejected")
            # print(BookingRequest.objects.filter(BookingRequest.objects.all()))
            all = BookingRequest.objects.all()
            print(all)
            print(requests)
            request_serializer = FetchBookingRequestSerializer(requests,many=True)
